...
def result(request):
    if request.method == "POST":
        ...
        Kanker.objects.create(
        rerata_jari2_lobus=rerata_jari2_lobus,
        rerata_tumor_inti=rerata_tumor_inti,
        rerata_luas_lobus=rerata_luas_lobus,
        rerata_luas_permukaan_tumor=rerata_luas_permukaan_tumor,
        rerata_cekungan_kontur=rerata_cekungan_kontur,
        rerata_jumlah_cekungan_kontur=rerata_jumlah_cekungan_kontur,
        se_jari2_lobus=se_jari2_lobus,
        se_tekstur_permukaan=se_tekstur_permukaan,
        se_tumor_inti=se_tumor_inti,
        se_luas_permukaan_tumor=se_luas_permukaan_tumor,
        se_cekungan_kontur=se_cekungan_kontur,
        se_jumlah_cekungan_kontur=se_jumlah_cekungan_kontur,
        se_fraktal_spesimen=se_fraktal_spesimen,
        keparahan_jari2_lobus=keparahan_jari2_lobus,
        keparahan_tekstur_permukaan=keparahan_tekstur_permukaan,
        keparahan_tumor_inti=keparahan_tumor_inti,
        keparahan_luas_lobus=keparahan_luas_lobus,
        keparahan_luas_permukaan_tumor=keparahan_luas_permukaan_tumor,
        keparahan_cekungan_kontur=keparahan_cekungan_kontur,
        keparahan_jumlah_cekungan_kontur=keparahan_jumlah_cekungan_kontur,
        diagnosis=result[0]
        )
        return render(request, 'result.html',{'result': 'predict'})
...
#     model = pd.read_pickle("./models/model2.pickle")
...
#             try:
#                 form.save()
#                 return redirect('/predict/result', {'result':result})
#             except:
#                 pass
#     else:
#         # form = KankerForm()
...
## PDF
class KankerToPdf(View):
    def get(self,request):
        var = {
            'kanker' : Kanker.objects.values(
            
            ),
            'request': request
        }
        return Render.to_pdf(self,'kanker_to_pdf.html', var)